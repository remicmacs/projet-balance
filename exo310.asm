...
RES_VECT  CODE    0x0000            ; processor reset vector
    GOTO    DAWN                   ; go to beginning of program
...
HIGH_ISR
    BCF INTCON, 1
    ; On complémente MARCHE_ARRET et on toggle le bit de marche/arrêt du timer
    COMF MARCHE_ARRET
    BTG T0CON, 7
    RETFIE  FAST
...
DAWN
    ; Initialisation de MARCHE_ARRET à 255
    SETF MARCHE_ARRET    
    ; Réglage de l'horloge
    BSF OSCCON, 6
    BCF OSCCON, 5
    BSF OSCCON, 4
    ; On met le port D en mode sortie
    CLRF TRISD
    ; Réglage du TIMER0 (62500 tics pour 1s)
    ; 0: TIMER0 arrêté
    ; 0: 16 bits
    ; 0: Internal
    ; 1: Front montant
    ...
    ; On initialise le port D et WREG
    MOVLW b'10101010'
    MOVWF PORTD
    CALL WAIT

LED_LOOP
    ; On boucle en alternant les LEDs
    RLNCF PORTD, 1
    CALL WAIT
    BRA LED_LOOP
    
WAIT
    ; On charge la valeur de départ du timer
    MOVLW 0x0B
    MOVWF TMR0H
    MOVLW 0xDB
    MOVWF TMR0L

    ; On met réinitialise le flag d'interruption et on démarre le timer
    BCF INTCON, 2
    BSF T0CON, 7

BIT_WAIT
    ; On boucle tant que le flag d'interruption n'est pas mis à 1
    BTFSS INTCON, 2
    BRA BIT_WAIT
